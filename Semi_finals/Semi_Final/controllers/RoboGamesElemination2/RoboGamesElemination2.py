...
def wait_3_sec() : #Stop the robot for a 0.5 seconds.
    print("Survivor found")
    start_time1 = robot.getTime()
    while robot.step(TIME_STEP) != -1:
        if robot.getTime() - start_time1 > 3:
            break
...
def explore():
    ...
    while robot.step(TIME_STEP) != -1:
        ...
        if detect_color(camera1)=="G" and list_ps[1].getValue() > 130:
            if (x,y) not in survivor:
                survivor.append((x,y))
                print(survivor)    
        ...
        # Calculate the direction to backtrack
            ...
            print("Please stop the simulation and place the robot to the correct direction.")
            break
...
def find_optimal_path(maze, start, greenAgents):
    import heapq
    print("finding survivor path")

    # Ensure start is a tuple (convert if needed)
    start = tuple(start)

    def dijkstra(maze, start, goal):
        # Convert start and goal to tuples in case they're passed as lists
        start = tuple(start)
        goal = tuple(goal)
    ...
    def solve_maze(maze, start, green_cells):
        # Convert green_cells to a set of tuples
        green_cells = set(map(tuple, green_cells))
        
        order = [start]
        unvisited = green_cells
        current = start
        final_path = []
    ...
    print("survivor path found")

    return solution_path, total_cost

    
def survivor_run(stack):
    reversed_stack = stack[::-1]
    reversed_stack.pop()
    print("starting survivor run")
    while robot.step(TIME_STEP) != -1:
        (x,y) = robot_position
        for i in range(3):
            if (x,y) == survivor[i]:
                wait_3_sec() #function for wait 3sec
...
explore()
wait_3_sec()
robot_position = duplicate_position
robot_direction = duplicate_direction
(path,cost) = find_optimal_path(cells,robot_position,survivor)
enter()
survivor_run(path)
